...
struct {
  ...
  float light[30]; // 120 bytes, 131 in total
  float temp[30]; // 120 bytes, 251 in total
  float humid[30]; // 120 bytes, 371 in total
  uint8_t moist[30]; // 30 bytes, 401 in total
  uint8_t padding[3];  // 3 byte,  404 in total
} rtcData;
bool rtcValid = false;
...
void loop() {
  ...
  if (wakeCounter >= 3) {
    ...
    while (wifiStatus != WL_CONNECTED) {
      ...
      if (retries == 300) {
        // Giving up after 15 seconds - taking measurements and writing to RTC before sleeping - will try in 30 mins time.  
    ...
    unsigned long deltaWifi = wifiStart - wifiEnd;
    Serial.print("WiFi finished in ");
    Serial.println(deltaWifi);
    client.setServer(mqtt_server, 1883); //Initialising MQTT Connection
    unsigned long readStart = micros();
    readvalues(); //Reads values from sensors
    unsigned long readEnd = micros();
    unsigned long delta = readStart - readEnd;
    Serial.print("Read finished in ");
...
void sendMQTTmessage()
{
  ...
  client.publish("moneyplant/telems/humidity", String(humidity).c_str(), false);
  client.publish("moneyplant/telems/temperature", String(temperature).c_str(), false);
  client.publish("moneyplant/telems/light", String(lux).c_str(),false);
  client.publish("moneyplant/telems/soilmoisture", String(soilMoistureValue).c_str(),false);
...
void WriteRTCValues() {
  ...
  rtcData.light[wakeCounter] = lux;
  rtcData.temp[wakeCounter] = temperature;
  rtcData.humid[wakeCounter] = humidity;
  rtcData.moist[wakeCounter] = soilMoistureValue;
